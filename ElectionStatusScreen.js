...
export default function ElectionStatusScreen() {
  ...
  const parseDateTime = (dateStr, timeObj) => {
    ...
    if (typeof timeObj === "string" && dateStr) return new Date(`${dateStr}T${timeObj}:00`);
    return null;
  };
  ...
  useEffect(() => {
    ...
    const unsubscribeElection = onSnapshot(
      ...
      (error) => console.log("Election listener stopped:", error.message)
    );
    unsubscribers.push(unsubscribeElection);
    ...
    const unsubscribeCandidates = onSnapshot(
      ...
      (error) => console.log("Candidates listener stopped:", error.message)
    );
    ...
    const unsubscribeVotes = onSnapshot(
      ...
      (error) => console.log("Votes listener stopped:", error.message)
    );
    unsubscribers.push(unsubscribeVotes);

    // cleanup
    return () => {
      unsubscribers.forEach((unsub) => unsub());
    };
  }, [eventId, userVotes, isLoggedIn]);
  ...
  if (!isLoggedIn) {
    return (
      <View style={styles.centered}>
        <Text style={styles.loggedOutText}>Please log in to view election data.</Text>
      </View>
    );
  }
                ...
                >
                  <View style={styles.candidateLeft}>
                    <View style={styles.avatarContainer}>
                      {candidate.photoURL ? (
                        <Image source={{ uri: candidate.photoURL }} style={styles.avatar} resizeMode="cover" />
                      ) : (
...
const styles = StyleSheet.create({
  ...
  loggedOutText: { fontSize: 16, color: "#555" },
