...
export default function ElectionsScreen() {
  ...
  // Listen to elections only if user is logged in
  useEffect(() => {
    if (!user) {
      // If logged out, stop everything and reset state
      setElections({});
      setEventInfo(null);
      setEventId(null);
      setLoading(false);
    ...
    const unsubscribeElections = onSnapshot(
          ...
          const unsubscribeCandidates = onSnapshot(
            ...
            (candidatesSnap) => {
              ...
              candidatesSnap.forEach((candidateDoc) => {
                ...
                updatedGrouped[candidateData.position] =
                  updatedGrouped[candidateData.position].filter(
            ...
            (error) => {
              console.warn("Candidates listener error:", error.message);
            }
          );

          candidateUnsubs.push(unsubscribeCandidates);
        });
      ...
      (error) => {
        console.warn("Elections listener error:", error.message);
        setLoading(false);
      }
    );
  ...
  const handleSubmitAllVotes = async () => {
    try {
      ...
      const missing = allPositions.filter(
  ...
  return (
    <View style={styles.container}>
      <ScrollView ref={scrollViewRef}>
        ...
        <TouchableOpacity
          style={[styles.submitButton, !isActive && { backgroundColor: "#999" }]}
